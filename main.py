...
os.environ["OPENAI_API_KEY"] = "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"
...
def embedding():
    ...
    if 'embedding_initialized' not in st.session_state or not st.session_state.embedding_initialized:
        if 'recursive_query_engine' not in st.session_state:
            ...
            # kiểm tra saved_index có tồn tại chưa, nếu chưa có thì lưu vào, nếu có thì sử dụng không cần embeding lại
            if os.path.exists('saved_index'):
                ...
                recursive_index = VectorStoreIndex(nodes=base_nodes+objects)
                recursive_index.storage_context.persist("saved_index")
                raw_index = VectorStoreIndex.from_documents(documents)
...
if not st.session_state.df_filtered_data.empty:
    ...
    previous_table = st.empty()  # Clear previous table
    previous_table.table(st.session_state.df_filtered_data)  # Display new table
